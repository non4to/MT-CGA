...
def put_data_together(rootLog:str):
    ...
    for execution in rootLogPath.iterdir():
        if "seed" not in str(execution): continue
        ...
        if experimentName not in taskMap:
            taskMap[experimentName] = {}
            taskMapJsonPath = os.path.join(execution,"grid_taskMap.json")
            with open(taskMapJsonPath, "r") as f:
                taskMap[experimentName] = json.load(f)
    ...
    with open(os.path.join(rootLogPath,f"taskMaps.json"), 'w') as f:
        json.dump(taskMap, f)
    end3=time.time()    
...
def load_parquet_log(archivePath:str):
    ...
    df["shape"] = df["shape"].apply(lambda s: np.stack(s).astype(int))
...
def merge_genotypic_experiment_csvs(base_dir):
    base_dir = Path(base_dir)
    if not base_dir.exists():
        raise FileNotFoundError(f"Directory not found: {base_dir}")
    prefixes = ("mixed-", "bridge-", "walker-")
    representatives_dfs = []
    speciation_dfs = []
    ...
    for folder in base_dir.iterdir():
        if not folder.is_dir():
            continue
        if not folder.name.startswith(prefixes):
            continue
        print(f"Processing: {folder.name}")

        representatives_file = folder / "representatives.csv"

        if representatives_file.exists():
            df = pd.read_csv(representatives_file)
            df["experiment"] = folder.name
            representatives_dfs.append(df)
        else:
            print("WARNING: representatives.csv not found")

        speciation_file = folder / "speciation.csv"
        if speciation_file.exists():
            df = pd.read_csv(speciation_file)
            df["experiment"] = folder.name
            speciation_dfs.append(df)
        else:
            print("  WARNING: speciation.csv not found")

    if representatives_dfs:
        representatives_all = pd.concat(
            representatives_dfs,
            ignore_index=True
        )

        output_file = base_dir / "genotypic_representatives_all.csv"
        representatives_all.to_csv(
            output_file,
            index=False
        )
        ...
        print(
            f"\nSaved: {output_file}"
            f"\nRows: {len(representatives_all)}"
        )

    if speciation_dfs:
        speciation_all = pd.concat(
            speciation_dfs,
            ignore_index=True
        )

        output_file = base_dir / "genotypic_speciation_all.csv"
        speciation_all.to_csv(
            output_file,
            index=False
        )
        print(
            f"\nSaved: {output_file}"
            f"\nRows: {len(speciation_all)}"
        )

    print("\nDone!")

def merge_phenotypic_experiment_csvs(base_dir):
    base_dir = Path(base_dir)
    if not base_dir.exists():
        raise FileNotFoundError(f"Directory not found: {base_dir}")
    prefixes = ("mixed-", "bridge-", "walker-")
    ...
    for folder in base_dir.iterdir():
        ...
        representatives_file = folder / "phenotypic_representatives.csv"
        ...
        if representatives_file.exists():
            ...
            print("WARNING: phenotypic_representatives.csv not found")

        speciation_file = folder / "phenotypic_speciation.csv"
        if speciation_file.exists():
            df = pd.read_csv(speciation_file)
            df["experiment"] = folder.name
            speciation_dfs.append(df)
        else:
            print("  WARNING: phenotypic_speciation.csv not found")
    ...
    if representatives_dfs:
        ...
        output_file = base_dir / "phenotypic_representatives_all.csv"
        representatives_all.to_csv(
            output_file,
            index=False
        )
    ...
    if speciation_dfs:
        ...
        output_file = base_dir / "phenotypic_speciation_all.csv"
        speciation_all.to_csv(
            output_file,
            index=False
        )
...
if __name__=="__main__":
    #  put_data_together(rootLog="log")
    # Define os prefixos das pastas
    merge_phenotypic_experiment_csvs("/home/flalipe/Projects/Airob/log")

    # Descobre todas as pastas que iniciam com os prefixos
    # df, _ , _ = load_parquet_log("log/completeData.parquet")
    # print(df.head)
    # print(df.columns)
    #  put_data_together(rootLog="log/v4-3000gens")
    # df, _, _ = load_parquet_log("log/v1/completeData.parquet")
    # print(df.head())
