import numpy as np, re, ast, os, json
from Analysis.data import loaders
    ...
    if toroidal:
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                ...
                neighPosX = (x + i) % cols
                neighPosY = (y + j) % rows
                # neighPosX = (x + i) % rows [old]
                # neighPosY = (y + j) % cols [old]
        ...
        for i in [-1,0,1]:
            for j in [-1,0,1]:
                ...
                elif (neighPosX > cols-1 or neighPosY > rows-1): continue
                output.append((neighPosX,neighPosY))  
        return output

def hamming_distance(shape1: list, shape2: list) -> float:
    """Returns hamming distance between the shape of two robots. (0-> same \ 1-> completely different)"""
...
def pos_to_task(pos: tuple, taskmap: dict) -> str:
        """Given pos, returns task"""
        x, y = pos
        key = f"({x},{y})"
        return taskmap[key]

def get_seed(experiment_name: str) -> int:
    """gets seed from experiment string."""
    match = re.search(r'_seed(\d+)_', experiment_name)
    return int(match.group(1))

def id_to_pos_gen(executionFolder:str):
    path = os.path.join(executionFolder, "robots_log.jsonl")
    id2PosGen = {}
    with open(path) as f:
        for line in f:
            entry = json.loads(line)
            id2PosGen[entry["id"]] = (tuple(entry["pos"]), entry["gen"])
    return id2PosGen

def parse_pos(pos) -> tuple:
    """transform pos into tuple!"""
    return ast.literal_eval(pos) if isinstance(pos, str) else pos

def parse_shape(shape):
    """Shape has to be a list of lists"""
    return ast.literal_eval(shape) if isinstance(shape, str) else shape
