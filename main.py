...
def main(world_types:list[str], robot_type:str, save_interval:int=1, seed:int=7, sim_step:int=400, max_gen:int=400,
    ...
    numprocs:int=5, mut_chance: float=0.05, cga_toroid:bool=False, botparams:dict={}):
    ...
    if (robot_type=="matteobot") and not(botparams):
        raise ValueError("Bot params is empty!")
    ...
    generationFolder = f"{prefix}{os.sep}bots"
    os.makedirs(generationFolder, exist_ok=True)
    problematicBots = f"{prefix}{os.sep}badbots" #this folder was created to save bots that when through some problem in evaluation
    os.makedirs(problematicBots, exist_ok=True)
    shutil.copy("parameters.json", f"{prefix}{os.sep}parameters.json")

    #Progress Archive
    progressFilePath = f"{prefix}{os.sep}expProgress.txt"
    progressCheckPoints = 1#max(1, int(options.max_gen * 0.15)) #every 20%

    worlds = []
    if robot_type=="basicrobot":
        ...
        for gen in range(options.max_gen):
            if (gen % (progressCheckPoints)==0) or (gen==options.max_gen-1):
                with open(progressFilePath,"a") as pFile:
                    ...
                    pFile.write(line)
                    print(f"--- Checkpoint ({time.strftime('%H:%M:%S', time.localtime())}): {line} ---")
